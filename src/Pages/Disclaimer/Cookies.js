    ...
    <div className="bg-white min-h-screen py-16 px-4 md:px-8 lg:px-16 text-gray-800 font-fahkwang">
      <div className="max-w-5xl mx-auto space-y-10 mt-16">
        ...
        <p className="text-lg leading-relaxed text-gray-700">
          {t("at")} <span className="font-semibold text-black">MOONSHADE</span> {t("useCookiesDescription")}
        </p>

        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-black font-bodoni">{t("whatAreCookies")}</h2>
          <p className="text-lg text-gray-700">{t("whatAreCookiesDesc")}</p>

          <h2 className="text-2xl font-bold text-black font-bodoni">{t("howWeUseThem")}</h2>
          <ul className="list-disc list-inside space-y-2 text-lg text-gray-700">
            <li>{t("analyzeTraffic")}</li>
            <li>{t("storePreferences")}</li>
            <li>{t("personalizeExperience")}</li>
            <li>{t("securePerformance")}</li>
          </ul>

          <h2 className="text-2xl font-bold text-black font-bodoni">{t("manageCookies")}</h2>
          <p className="text-lg text-gray-700">{t("manageCookiesDesc")}</p>
        </div>
